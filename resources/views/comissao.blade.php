    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Comissões
                @can('insere_comissao')
                    <button type="button" class="btn btn-success btn-rounded float-right" data-toggle="modal"
                        data-target="#myModal">
                        <i class='fas fa-plus'></i> Comissao
                    </button>
                @endcan
                </h4>

                @include("modals.modal_comissao")
            </div>
            <div class="form-row col-lg-12">
                <div>
                    <a href="{{ url("/Cadastro/comissao") }}" class="btn btn-sm btn-info mt-3 mr-2">Todos</a>
                </div>
                <div class="form-group col-lg-2">
                    <select onchange="verificar(this.value)" class="form-control input-border-bottom" id="filtro"
                        ...
                        <option value="D">Data Pagamento</option>
                        <option value="P">Situação</option>
                        <option value="B">Status</option>
                    </select>
                </div>
                <div id="name" hidden>
                    <form action="{{ url('/Comissao/busca') }}" method="post">
                        <div class=" container">
                ...
                <div id="cod" hidden>
                    <form action="{{ url('/Comissao/busca2') }}" method="post">
                        <div class="container">
                ...
                <div id="pag_rec" hidden>
                    <form action="{{ url('/Comissao/busca3') }}" method="post">
                        <div class="container">
                ...
                <div id="conc" hidden>
                    <form action="{{ url('/Comissao/busca4') }}" method="post">
                        <div class="container">
                        <div class="container">
                            <div class="input-group col-lg-12 mt-2 mr-2" >
                                <select class="form-control input-border-bottom" id="criterio"
                    ...
                    <table id="multi-filter-select" class="display table table-striped table-hover text-center">
                        <thead>
                            <tr>
                                ...
                                <th class="">Data Prevista do Pagamento</th>
                                <th class="">Status</th>
                            ...
                            @foreach($comissao as $c)
                                @can('view_comissao', $c)
                                    <tr>
                                        ...
                                        <td class="">{{ date('d-m-Y', strtotime($c->Data_Prev)) }} </td>
                                        <td class=""> {{ $c->Status=="A"?"Aberto":"Pago"}} </td>
                                        <td class=""> {{ $c->Situacao=="L"?"Livre":"Boloqueado" }} </td>
                                        <td> {{ $c->user_id }} </td>
                                        <td class="">
                                            <div class="btn-group" role="group">
                                            @can('edita_comissao')
                                                <a href='{{ url("/Comissao/editar/$c->Codigo") }}'
                                                class="btn btn-success btn-xs mr-2" style="border-radius:2px;"><i class='far fa-edit'></i></a>
                                            @endcan
                                            @can('visual_comissao')
                                            <a href='{{ url("/Comissao/visualizar/$c->Codigo") }}'
