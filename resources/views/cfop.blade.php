    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">CFOP
                @can('insere_cfop')
                    <button type="button" class="btn btn-success btn-rounded float-right" data-toggle="modal"
                        data-target="#myModal">
                        <i class='fas fa-plus'></i> CFOP
                    </button>
                @endcan
                </h4>
            @include("modals.modal_cfop")
              
            </div>
            <div class="form-row col-lg-12">
                <div>
                    <a href="{{ url("/Cadastro/cfop") }}" class="btn btn-sm btn-info mt-3 mr-2">Todos</a>
                </div>
                <div class="form-group col-lg-2">
                    <select onchange="verificar(this.value)" class="form-control input-border-bottom" id="filtro"
                        ...
                        <option value="D">CFOP</option>
                    </select>
                </div>
                <div id="name" hidden>
                    <form action="{{ url('/CFOP/busca') }}" method="post">
                        <div class=" container">
                            <div class="input-group col-lg-10 mt-2">
                                <input type="text" class="form-control" name="criterio" placeholder="CFOP">
                ...
                <div id="cod" hidden>
                    <form action="{{ url('/CFOP/busca2') }}" method="post">
                        <div class="container">
                        ...
                        @foreach($cfop as $f)
                            @can('view_cfop', $f)
                                <tr>
                                    ...
                                    <td class="">{{ $f->CFOP}}  </td>
                                    <td class="">{{ $f->Descricao }}  </td>
                                    <td class="">{{ $f->Aplicacao }}  </td>
                                    <td class="">{{ $f->user_id }}  </td>
                                    <td class="">
                                        <div class="btn-group" role="group">
                                        @can('edita_cfop')
                                            <a href='{{ url("/CFOP/editar/$f->Codigo") }}'
                                            class="btn btn-success btn-xs mr-2" style="border-radius:2px;"><i class='far fa-edit'></i></a>
                                        @endcan
                                        @can('visual_cfop')
                                                        <a href='{{ url("/CFOP/visualizar/$f->Codigo") }}'
