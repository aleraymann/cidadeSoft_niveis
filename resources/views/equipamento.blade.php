...
<script type="text/javascript">
    jQuery(function ($) {
        $("#Nro_Frota").mask("99999");
        $("#Fabricacao").mask("9999");
    });
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Equipamento
                @can('insere_equipamento')
                    <button type="button" class="btn btn-success btn-rounded float-right" data-toggle="modal"
                        data-target="#myModal">
                        <i class='fas fa-plus'></i> Equipamento
                    </button>
                @endcan
                </h4>
              @include('modals.modal_equipamento')
            </div>
           
            <div class="form-row col-lg-12">
                <div>
                    <a href="{{ url("/Cadastro/equipamento") }}" class="btn btn-sm btn-info mt-3 mr-2">Todos</a>
                </div>
                ...
                <div id="name" hidden>
                    <form action="{{ url('/Equipamento/busca') }}" method="post">
                        <div class=" container">
                ...
                <div id="cod" hidden>
                    <form action="{{ url('/Equipamento/busca2') }}" method="post">
                        <div class="container">
                ...
                <div class="table-responsive">
                    <table id="multi-filter-select" class="display table table-striped table-hover text-center">
                        ...
                        <tbody>
                            ...
                            @can('view_equipamento', $e)
                                <tr>
                                    <td class="">{{ $e->Codigo }}  </td>
                                    @if($e->Foto != null)
                                            <td><img src="{{ url("storage/equipamentos/{$e->Foto}") }}" style="max-width:30px; height:30px" ></td>
			                            @else
				                        <td> <img src="{{url("img/empresa_padrao.jpg")}}"  style="max-width:30px; height:30px"></td>
			                            @endif
                                    <td class="">{{ $e->Equipamento }}  </td>
                                    <td class="">{{ $e->user_id }}  </td>
                                    <td class="">
                                        <div class="btn-group" role="group">
                                        @can('edita_equipamento')
                                            <a href='{{ url("/Equipamento/editar/$e->Codigo") }}'
                                            class="btn btn-success btn-xs mr-2" style="border-radius:2px;"><i class='far fa-edit'></i></a>
                                        @endcan
                                        @can("visual_equipamento")
                                                        <a href='{{ url("/Equipamento/vizualizar/$e->Codigo") }}' class="btn btn-secondary btn-xs mr-2" style="border-radius:2px;">
                                                            <i class='far fa-eye'></i>
                                                        </a>
                                                    @endcan
                                        @can('deleta_equipamento')
                                                <a href="javascript:deletarRegistro('{{ $e->Codigo }}')"
...
<script>
    function deletarRegistro(id) {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: "{{ url("Equipamento/excluir") }}" + '/' + id,
                    type: 'DELETE',
                    data: {
                        '_method': 'DELETE',
                        '_token': csrf_token
