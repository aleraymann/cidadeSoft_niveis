    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Contas a Receber
               @can('insere_ctas_receber')
                ...
                @include("modals.modal_contas_receber")
            </div>
            <div class="form-row col-lg-12">
                <div>
                    <a href="{{ url("/Cadastro/ctas_receber") }}" class="btn btn-sm btn-info mt-3 mr-2">Todos</a>
                </div>
                <div class="form-group col-lg-2">
                    <select onchange="verificar(this.value)" class="form-control input-border-bottom" id="filtro"
                        ...
                        <option value="D">Vencimento</option>
                    </select>
                </div>
                <div id="name" hidden>
                    <form action="{{ url('/Contas_Receber/busca') }}" method="post">
                        <div class=" container">
                ...
                <div id="cod" hidden>
                    <form action="{{ url('/Contas_Receber/busca2') }}" method="post">
                        <div class="container">
                            ...
                            @foreach($ctas_receber as $c)
                               @can('view_ctas_receber', $c)
                                    <tr>
                                        ...
                                        <td class=""> {{ date('d-m-Y', strtotime($c->Vencimento)) }} </td>
...
<script>
    function deletarRegistro(id) {
        ...
        swal({
            ...
            if (willDelete) {
                $.ajax({
                    url: "{{ url("Contas_Receber/excluir") }}" + '/' + id,
                    type: 'DELETE',
                    data: {
                        '_method': 'DELETE',
                        '_token': csrf_token
