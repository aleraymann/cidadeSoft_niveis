    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Telemarketing
                @can('insere_tele')
                ...
                @include("modals.modal_telemarketing")
            </div>
            <div class="form-row col-sm-12">
                <div>
                    <a href="{{ url("/Cadastro/telemarketing") }}" class="btn btn-sm btn-info mt-3 mr-2">Todos</a>
                </div>
                <div class="form-group col-lg-2">
                    <select onchange="verificar(this.value)" class="form-control input-border-bottom" id="filtro"
                        ...
                        <option value="D">Data do Contato</option>
                        <option value="P">Cod do Responsável</option>
                        <option value="B">Concluido</option>
                    </select>
                </div>
                <div id="name" hidden>
                    <form action="{{ url('/Telemarketing/busca') }}" method="post">
                        <div class=" container">
                ...
                <div id="cod" hidden>
                    <form action="{{ url('/Telemarketing/busca2') }}" method="post">
                        <div class="container">
                ...
                <div id="pag_rec" hidden>
                    <form action="{{ url('/Telemarketing/busca3') }}" method="post">
                        <div class="container">
                ...
                <div id="conc" hidden>
                    <form action="{{ url('/Telemarketing/busca4') }}" method="post">
                        <div class="container">
                        <div class="container">
                            <div class="input-group col-lg-12 mt-2 mr-2" >
                ...
                <div class="table-responsive">
                    <table id="multi-filter-select" class="display table table-striped table-hover text-center">
                        <thead>
                            <tr>
                                <th class="">Cod</th>
                                <th class="">Data do Contato</th>
                                <th class="">Cod do Resposável</th>
                        ...
                        <tbody>
                            @foreach($telemarketing as $t)
                                    @can('view_telemarketing', $t)
                                        <tr>
                                            <td class=""> {{ $t->Codigo }} </td>
                                            <td class="">{{ date('d-m-Y', strtotime($t->Data)) }}</td>
                                            <td> {{ $t->Cod_Func }} </td>
                                            <td> {{ $t->Concluso=="1"? "Sim":"Não" }} </td>
                                            <td> {{ $t->user_id }} </td>
                                            <td class="">
                                                <div class="btn-group" role="group">
                                                    @can('edita_tele')
                                                    <a href='{{ url("/Telemarketing/editar/$t->Codigo") }}' class="btn btn-success btn-xs mr-2" style="border-radius:2px;">
                                                            <i class='far fa-edit'></i>
                                                        </a>
                                                    @endcan
                                                    @can('view_tele')
                                                    <a href='{{ url("/Telemarketing/visualizar/$t->Codigo") }}' class="btn btn-secondary btn-xs mr-2" style="border-radius:2px;">
                                                            <i class='far fa-eye'></i>
                                                        </a>
                                                    @endcan
                                                    @can('deleta_tele')
                                                    <a href="javascript:deletarRegistro('{{ $t->Codigo }}')" class="btn btn-danger btn-xs" style="border-radius:2px;">
    ...
    <script>
        function deletarRegistro(id) {
            ...
            swal({
                ...
                if (willDelete) {
                    $.ajax({
                        url: "{{ url("Telemarketing/excluir") }}" + '/' + id,
                        type: 'DELETE',
                        data: {
                            '_method': 'DELETE',
                            '_token': csrf_token
