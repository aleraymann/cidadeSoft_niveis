    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Duplicata
                @can('insere_duplicata')
                    <button type="button" class="btn btn-success btn-rounded float-right" data-toggle="modal"
                        data-target="#myModal">
                        <i class='fas fa-plus'></i> Duplicata
                    </button>
                @endcan
                </h4>

                @include("modals.modal_duplicata")
            </div>
            <div class="form-row col-lg-12">
                <div>
                    <a href="{{ url("/Cadastro/duplicata") }}" class="btn btn-sm btn-info mt-3 mr-2">Todos</a>
                </div>
                <div class="form-group col-lg-2">
                    <select onchange="verificar(this.value)" class="form-control input-border-bottom" id="filtro"
                        ...
                        <option value="D">Data de Vencimento</option>
                        <option value="P">Cod do Cli/For</option>
                        <option value="B">Cod do Boleto</option>
                    </select>
                </div>
                <div id="name" hidden>
                    <form action="{{ url('/Duplicata/busca') }}" method="post">
                        <div class=" container">
                ...
                <div id="cod" hidden>
                    <form action="{{ url('/Duplicata/busca2') }}" method="post">
                        <div class="container">
                ...
                <div id="pag_rec" hidden>
                    <form action="{{ url('/Duplicata/busca3') }}" method="post">
                        <div class="container">
                            <div class="input-group col-lg-12 mt-2 mr-2" >
                            <input type="text" class="form-control" name="criterio" placeholder="Cod do Cli/For">
                ...
                <div id="conc" hidden>
                    <form action="{{ url('/Duplicata/busca4') }}" method="post">
                        <div class="container">
                        <div class="container">
                            <div class="input-group col-lg-12 mt-2 mr-2" >
                            <input type="text" class="form-control" name="criterio" placeholder="Cod do Boleto">
                                <div class="input-group-append ml-2">
                                    <button class="btn btn-success" type="submit">OK</button>
                                </div>
                            </div>
                    ...
                    <table id="multi-filter-select" class="display table table-striped table-hover text-center">
                        <thead>
                            <tr>
                                <th class="">Cod</th>
                                <th class="">Fatura</th>
                                <th class="">Verncimento</th>
                                <th class="">Valor</th>
                                <th class="">Cod Cli/For</th>
                                <th class="">Cod Boleto</th>
                                <th class="">Cod Adm</th>
                            </tr>
                        </thead>
                        ...
                        <tbody>
                            @foreach($duplicata as $dup)
                                @can('view_duplicata', $dup)
                                    <tr>
                                    <td> {{ $dup->Codigo }} </td>
                                    <td> {{ $dup->Fatura }} </td>
                                    <td> {{ date('d-m-Y', strtotime($dup->Vencimento)) }} </td>
                                    <td> {{ $dup->Valor }} </td>
                                    <td> {{ $dup->Cod_CliFor }} </td>
                                    <td> {{ $dup->Cod_Boleto }} </td>
                                    <td> {{ $dup->user_id }} </td>
                                    <td>
                                            <div class="btn-group" role="group">
                                            @can('edita_duplicata')
                                                <a href='{{ url("/Duplicata/editar/$dup->Codigo") }}'
                                                class="btn btn-success btn-xs mr-2" style="border-radius:2px;"><i class='far fa-edit'></i></a>
                                            @endcan
                                            @can('visual_duplicata')
                                            <a href='{{ url("/Duplicata/visualizar/$dup->Codigo") }}'
                                            class="btn btn-secondary btn-xs mr-2" style="border-radius:2px;"><i class='far fa-eye'></i></a>
                                        @endcan
                                            @can('deleta_duplicata')
                                                    <a href="javascript:deletarRegistro('{{$dup->Codigo}}')"
